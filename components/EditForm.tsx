...
const EditForm = () => {
  ...
  useEffect(() => {
    if (getId !== undefined) {
      reset({
        customer: getId.customer_name,
        invoice: getId.invoice_id,
        date: getId.transaction_date.split("T")[0],
        subTotal: getId.subtotal,
        total_price: getId.total_price,
        tax: parseInt(getId.tax),
        discount: parseInt(getId.discount),
        rows: getId.detail.map((row: any) => ({
          name: row.name,
          price: row.price,
          qty: row.quantity,
